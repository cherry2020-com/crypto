...
from sim_card.gsmmodem.util import lineMatching
from .exceptions import CommandError, InvalidStateException, CmeError, CmsError, InterruptedException, TimeoutException, PinRequiredError, \
...
class GsmModem(SerialComms):
    ...
    def connect(self, pin=None):
        ...
        try:
            ...
            try:
                self.write('AT+CRC=0') # Enable extended format of incoming indication (optional)
                ...
                self._extendedIncomingCallIndication = False  # -->FALSE
    ...
    def write(self, data, waitForResponse=True, timeout=15, parseError=True, writeTerm='\r', expectedResponseTermSeq=None):
    ...
    def dial(self, number, timeout=15, callStatusUpdateCallbackFunc=None):
...
class Call(object):
    ...
    dtmfSupport = True # Indicates whether or not DTMF tones can be sent in calls
...
class IncomingCall(Call):
    ...
    def answer(self):
        ...
        """
        if self.ringing:
            self._gsmModem.write('ATA', waitForResponse=False)
            self.ringing = False
            self.answered = True
        return self    
